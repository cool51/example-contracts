...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
 ...
 modifier onlyOwner(){
   require(msg.sender==owner,"Sorry only owner is allowed");
   _;
 }

  modifier verifyCaller (address _address) { require (msg.sender == _address,"caller not verified"); _;}

  modifier paidEnough(uint _price) { require(msg.value >= _price,"not enough payement"); _;}
  ...
  modifier forSale(uint _sku){
    require(uint(items[_sku].state)==uint(State.ForSale),"Not for sale");
    _;
  ...
  modifier sold(uint _sku){
        require(uint(items[_sku].state)==uint(State.Sold),"Not sold ");
    _;
  }
  modifier shipped(uint _sku){
        require(uint(items[_sku].state)==uint(State.Shipped),"Not shipped");
    _;
  }
  modifier received(uint _sku){
            require(uint(items[_sku].state)==uint(State.Received),"Not received");
    _;
  }
  ...
  {
Item memory buyItem=items[_sku];
buyItem.buyer=msg.sender;
buyItem.seller.transfer(buyItem.price);
buyItem.state=State.Sold;
emit LogSold(_sku);
